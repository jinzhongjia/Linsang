...
zig build interop              # curl/OpenSSL x RSA/ECDSA x TLS 1.2/1.3
...
fuzz runner has a `StackTrace` type mismatch. The interoperability step requires
the `curl` and `openssl` command-line tools.
