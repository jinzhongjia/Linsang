...
pub const CertKeyPair = struct {
    ...
    pub const Signed = struct {
        scheme: proto.SignatureScheme,
        bytes: []const u8,
    };

    pub fn signMessage(c: *CertKeyPair, message: []const u8, rng: std.Random, out: []u8) !Signed {
        const bytes = switch (c.key.signature_scheme) {
            inline .ecdsa_secp256r1_sha256,
            .ecdsa_secp384r1_sha384,
            => |comptime_scheme| brk: {
                const Ecdsa = SchemeEcdsa(comptime_scheme);
                const key_pair = switch (comptime_scheme) {
                    .ecdsa_secp256r1_sha256 => c.ecdsa_key_pair.?.ecdsa_secp256r1_sha256,
                    .ecdsa_secp384r1_sha384 => c.ecdsa_key_pair.?.ecdsa_secp384r1_sha384,
                    else => unreachable,
                };
                ...
                if (out.len < der.len) return error.BufferUndersize;
                @memcpy(out[0..der.len], der);
                break :brk out[0..der.len];
            },
            inline .rsa_pss_rsae_sha256,
            .rsa_pss_rsae_sha384,
            .rsa_pss_rsae_sha512,
            => |comptime_scheme| brk: {
                const signature = try c.key.key.rsa.signOaep(
                    SchemeHash(comptime_scheme),
                    message,
                    null,
                    out,
                    rng,
                );
                break :brk signature.bytes;
            },
            .ed25519 => brk: {
                const signature = try c.key.key.ed25519.sign(message, null);
                const encoded = signature.toBytes();
                if (out.len < encoded.len) return error.BufferUndersize;
                @memcpy(out[0..encoded.len], &encoded);
                break :brk out[0..encoded.len];
            },
            else => return error.TlsUnknownSignatureScheme,
        };
        return .{ .scheme = c.key.signature_scheme, .bytes = bytes };
    }
